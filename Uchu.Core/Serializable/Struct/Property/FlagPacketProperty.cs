...
namespace Uchu.Core
{
    public class FlagPacketProperty : WrappedPacketProperty
    {
        ...
        public override void Read(object objectToWrite, BitReader reader, Dictionary<string, object> readProperties)
        {
            // Read the value if the bit is set to true.
            if (reader == null || !reader.ReadBit()) return;
            this.WrappedProperty.Read(objectToWrite, reader, readProperties);
        }
    }
}