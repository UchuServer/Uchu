...
namespace Uchu.World.Client
{
    public class ZoneParser
    {
        ...
        private readonly XmlSerializer _triggerSerializer = new XmlSerializer(typeof(Triggers));
        ...
        private async Task<ZoneInfo> ParseAsync(string path)
        {
            ...
            var zone = new ZoneInfo
            {
                ...
                Triggers = await GetTriggers(Path.GetDirectoryName(path))
            };
        ...
        private async Task<Trigger[]> GetTriggers(string path)
        {
            ...
            foreach (var file in files)
            {
                ...
                var triggers = (Triggers) _triggerSerializer.Deserialize(stream);
                ...
                foreach (var part in parts)
                {
                    ...
                    foreach (var trigger in triggers.TriggerElements)
                    {
                        trigger.PrimaryId = primaryId;
                    }
                    
                    triggerCollection.AddRange(triggers.TriggerElements);
